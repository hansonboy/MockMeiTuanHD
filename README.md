...
8. 应用程序之间互相调用，传参 
9. 遵守NSCoding协议的类对象中包含的属性对象也需要遵守NSCoding协议，这一点容易遗忘
10. 经常在数组中是否包含该对象，以及检查两个对象时是否相同的时候，都会调用isEqual:方法，我们可以重写该方法来达到根据内容而不是内存来判断两个对象是否相同 ，数组中遇到的所有需要比较两个对象的操作，都会调用isEqual：方法
11. 返回一个遵守协议的对象
12. 注意strong类型是引用，改一次就都改了哦，这个还是很重要的基础哦，在某些时候，可以发生某些想不到的功效或者负面的效果
13. 使用通知的优点：可以跨越好多层，但是缺点是：只要发出通知，那么所有的监听者都能够你听到，有时候我们可能并不希望这么做呢，通知有利有弊，选择合适的时机进行使用才好
14. 代理的回调的缺点是：在同一个Controller中如过发生多次代理回调的情况，需要写很多的if else语句，代码写起来耦合度很高，很恶心，使用回调的好处则是，每一个回调值调用对用的block执行就可以了，彼此之间真的是互不干扰，非常方便呢
